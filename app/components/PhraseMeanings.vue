...
function onClickCancel() {
  ...
  const meanings = phraseDetails.value?.meanings;
  if (meanings == undefined) return;
  const uploadedMedia = meanings[activeMenuIndex]?.media
...
function onClickConfirm() {
  ...
  const meanings = phraseDetails.value.meanings;
  if (meanings) {
    const clonedMeaning = structuredClone(toRaw(meanings[activeMenuIndex]))
    if (clonedMeaning !== undefined) {
      meanings[activeMenuIndex] = clonedMeaning;
    }
    editMeaning.value = false;
  }
}
